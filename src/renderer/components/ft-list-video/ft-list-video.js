...
export default defineComponent({
  ...
  computed: {
    ...
    // Get channel thumbnail from subscriptions if available
    channelThumbnail: function () {
      if (!this.channelId) {
        return null
      }

      // Look up channel thumbnail from active profile subscriptions
      const activeProfile = this.$store.getters.getActiveProfile
      if (activeProfile && activeProfile.subscriptions) {
        const subscription = activeProfile.subscriptions.find(
          sub => sub.id === this.channelId
        )
        if (subscription && subscription.thumbnail) {
          // Return appropriately sized thumbnail (36x36 for YouTube-style avatar)
          let thumbnail = subscription.thumbnail.replace(/=s\d+/, '=s72')

          // Handle Invidious backend
          if (this.backendPreference === 'invidious' && this.currentInvidiousInstanceUrl) {
            // Convert YouTube URL to Invidious if needed
            if (thumbnail.includes('yt3.ggpht.com') || thumbnail.includes('yt3.googleusercontent.com')) {
              thumbnail = thumbnail.replace(/https:\/\/yt3\.(ggpht|googleusercontent)\.com/, this.currentInvidiousInstanceUrl + '/ggpht')
            }
          }
